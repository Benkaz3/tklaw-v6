...
const renderOptions = {
  renderMark: {
    ...
    [MARKS.CODE]: text => <code className="bg-gray-100 p-1 rounded font-mono">{text}</code>,
  },
  renderNode: {
    ...
    [BLOCKS.LIST_ITEM]: (_node, children) => {
      ...
      const content = React.isValidElement(firstChild) && firstChild.props?.children
    ...
    [BLOCKS.EMBEDDED_ASSET]: node => {
      ...
      return (
        <figure className="mb-6">
          <img src={file.url} alt={title || ''} className="w-full h-auto rounded" />
          {title && (
        ...
        {authors.length === 1 && (() => {
          ...
          return (
            <aside className="flex flex-col sm:flex-row items-center bg-card_background rounded-lg shadow p-6">
              <figure className="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden flex-shrink-0 mb-4 sm:mb-0">
                <img
                  src={photoUrl}
                  alt={name}
              ...
              <div className="sm:ml-6 text-center sm:text-left">
                ...
                <Link
                  to={language === 'vi' ? `/vi/luat-su/${authorSlug}` : `/en/attorneys/${authorSlug}`}
